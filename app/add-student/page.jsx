...
export default function AddStudent() {
  ...
  const handleExtractMarks = async (marksType) => {
    setLoading((prev) => ({ ...prev, [marksType]: true }));
    setError("");

    const file = marksType === "class10th" ? formData.class10thMarksPdf : formData.class12thMarksPdf;

    if (!file) {
      setError(`Please upload the ${marksType} marksheet first.`);
      setLoading((prev) => ({ ...prev, [marksType]: false }));
      return;
    }

    const data = new FormData();
    data.append("marksheet", file);
    data.append("marksType", marksType);

    try {
      const res = await fetch("/api/extract-marks", {
        method: "POST",
        body: data,
      });
      ...
      if (res.ok) {
        setFormData((prev) => ({
          ...prev,
          [`${marksType}Marks`]: result.marks,
        }));
        alert(`Successfully extracted ${result.marks}% for ${marksType}.`);
      } else {
        setError(result.error || `Failed to extract marks for ${marksType}.`);
      }
    } catch (error) {
      setError("An error occurred while extracting marks.");
      console.error(error);
    } finally {
      setLoading((prev) => ({ ...prev, [marksType]: false }));
    }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      type="button"
                      onClick={() => handleExtractMarks("class10th")}
                      ...
                      disabled={loading.class10th}
                    >
                      {loading.class10th ? "Extracting..." : "Extract"}
                    </button>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      type="button"
                      onClick={() => handleExtractMarks("class12th")}
                      className="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2"
                      disabled={loading.class12th}
                    >
                      {loading.class12th ? "Extracting..." : "Extract"}
                    </button>
                  </div>
                </div>
